# Order Processor Service (order-processor)

**程序名称**: Order Processor Service  
**简称**: order-processor
...
5. **停止程序**：按 `Ctrl+C` 或使用管理脚本 `manage.ps1 stop`
...
- **start**: 启动 xlsx-converter 服务
- **stop**: 停止 xlsx-converter 服务
- **restart**: 重启 xlsx-converter 服务
- **status**: 查看服务运行状态、进程信息和日志文件信息
- **logs**: 查看最近的日志内容（最后20行）

### 日志

程序运行日志会保存到 `order_processor.log` 文件中，同时也会在控制台显示。
...
可以在 `order_processor.py` 文件中修改以下配置：
...
- 大文件转换可能需要一些时间
- 如果转换失败，原文件不会被删除
- 如果原文件正在被其他程序打开，删除操作可能会失败（程序会重试3次）
